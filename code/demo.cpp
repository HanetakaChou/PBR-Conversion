...
#include "_internal_full_screen_vertex.inl"
	...
	this->m_conversion_fs = NULL;
	{
#include "_internal_conversion_fragment.inl"
		HRESULT res_device_create_pixel_shader = device->CreatePixelShader(conversion_fragment_shader_module_code, sizeof(conversion_fragment_shader_module_code), NULL, &this->m_conversion_fs);
		assert(SUCCEEDED(res_device_create_pixel_shader));
	}

	this->m_shading_fs = NULL;
	{
#include "_internal_shading_fragment.inl"
		HRESULT res_device_create_pixel_shader = device->CreatePixelShader(shading_fragment_shader_module_code, sizeof(shading_fragment_shader_module_code), NULL, &this->m_shading_fs);
		assert(SUCCEEDED(res_device_create_pixel_shader));
	}
}
...
void Demo::Tick(ID3D11Device *device, ID3D11DeviceContext *device_context, ID3D11RenderTargetView *swap_chain_back_buffer_rtv, uint32_t swap_chain_image_width, uint32_t swap_chain_image_height)
{
	// Shading
	{
		...
		device_context->PSSetShader(this->m_conversion_fs, NULL, 0U);
	...
	// Shading
	{
		...
		device_context->VSSetShader(this->m_full_screen_vs, NULL, 0U);
		device_context->PSSetShader(this->m_shading_fs, NULL, 0U);
...
void Demo::Destroy(ID3D11Device *device, ID3D11DeviceContext *device_context)
{
	...
	this->m_conversion_fs->Release();
	this->m_shading_fs->Release();
}